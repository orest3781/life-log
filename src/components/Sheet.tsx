    ...
    >
      ...
      >
        <header className="flex items-center gap-2 px-5 pb-3 pt-3">
          <span
            aria-hidden
            className="absolute left-1/2 top-2 mx-auto h-1.5 w-10 -translate-x-1/2 rounded-full bg-ink"
          />
          <h2 className="mt-2 flex-1 t-sheet-title text-ink">{title}</h2>
          {headerAction}
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="anim-scale-in min-h-0 flex-1 overflow-y-auto px-5">
          {children}
        </div>
